...
\textbf{Key Takeaway:} When adding noise to the simulations, we see a significant de
...
do significantly change the outcome of galaxy evolution and the properties that we observe in galaxies at lower redshift - most importantly our Milky Way - or if the. Simulations can provide us with important insights to understand if mergers are 
...
\subsection{How comparable are the NIHAO Milky Way analogue and the Milky Way?}
...
\begin{itemize}
    ...
    \item Fig.~\ref{fig:FeH_XFe},
    \item Fig.~\ref{fig:NaFe_MgMn_selection_Age_FeH_dissection},
    ...
    \item Plotting elemental abundances against age however, proved fruitful, with very distinct sequences of accreted and in situ populations appearing in the simulation data (Fig.~\ref{fig:age_xfe_xh_nxh}) with up to 12-sigma separation (Tab.~\ref{tab:tabular_separation_r12}).
    \item Such sequences are not as clear in the observation-based plots (Fig.~\ref{fig:age_xfe_xh_nxh_galah}), as a result of massive uncertainties that come with the probabilistic nature of estimating stellar ages through isochrone fitting.
    \item Fig.~\ref{fig:histograms_xh_in_age_bins_Mn},
    \item When testing the significance of separations of sequences in the age-[X/H] plane, we see the largest separations for the youngest accreted stars, even at larger observational noise (Fig.~\ref{fig:r12_for_sigmaobs} and Tab.~\ref{tab:r_values_simulation})
    \item Fig.~\ref{fig:nihao_with_scatter}, 
    \item Fig.~\ref{fig:age_sigma_vs_r}
\end{itemize}
