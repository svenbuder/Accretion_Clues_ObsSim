...
Sequence~2 with \input{tex_text/sequence_2_fraction} of particles in the footprint shows the behaviour of kinematic halo stars with a spatial distribution not restricted to the galactic plane (Fig.~\ref{fig:three_sequences_traced}g) and angular momentum (or here: azimuthal velocity) consistent with almost no net rotation (Fig.~\ref{fig:three_sequences_traced}h). While \citet{Buder2022} found $V_\phi = {20}_{-70}^{+100}\,\mathrm{kpc\,km\,s^{-1}}$ for the major accretion remnant in the Milky Way, we find \input{tex_text/v_phi_sequence_2}. Once again, we notice the offset towards lower abundance enhancement of this sequence in the [Fe/H] vs. [Si/Fe] plane as well as the [Al/Fe] vs. [Mg/Mn] plane with respect to sequence 1 (Figs.~\ref{fig:three_sequences_traced}i and j, respectively).
