...
\begin{itemize}
    \item intrinsic scatter in {[X/H]} is larger ($\sim 0.1-0.4\,\mathrm{dex}$) for older stars as a result of larger change in this logarithmic scale when compared to youngest stars ($\sim 0.02-0.05\,\mathrm{dex}$).
...
\begin{table*}
    \centering
    \caption{\textbf{Separation significance $r_{12}$ between Sequence 1 and 2 as defined in Eq.~\ref{eq:r_value} for changing observational noise for the observable footprint.}}
    % Note: Currently tiny font. Can move sigma values to next rows if tiny font is not accepted.
    \tiny
