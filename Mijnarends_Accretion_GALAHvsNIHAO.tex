...
Naïvely, we initially plotted the diagnostic plots of observers, namely the ratio of SNII to SNIa via [Si/Fe] as function of iron abundance [Fe/H] (Fig.~\ref{fig:low_alpha_halo}a) as well as the previously succesful selection plane of [Al/Fe] vs. [Mg/Mn] (Fig.~\ref{fig:low_alpha_halo}b) for the complete Milky Way analogue. Our hope was, that in the absence of observational noise, we would be able to identify clearly separated areas of in-situ and accreted stars. To our big surprise, the [Al/Fe] vs. [Mg/Mn] showed seemingly even more scatter than the observations. Tracing the selection of accreted stars, that is, the stars with lower [Si/Fe] around $\mathrm{[Fe/H]} \sim -0.6$ with a selection box, does not lead to a unique location in the [Al/Fe] vs. [Mg/Mn] in Fig.~\ref{fig:low_alpha_halo}b when showing the extend of these stars with a convex hull outline.

Before giving up on our endeavour, we applied a similar selection to NIHAO as would be expected from GALAH observations, in particular the limiting of the sample to be within $4.2\,\mathrm{kpc}$ of the Solar radius while leaving out significant fractions of the galactic plane. Surprisingly, both the [Fe/H] vs. [Si/Fe] plane in Fig.~\ref{fig:low_alpha_halo}c as well as the [Al/Fe] vs. [Mg/Mn] plane in Fig.~\ref{fig:low_alpha_halo}d cleared up significantly, with the accreted sequence becoming more prominent around $\mathrm{[Al/Fe]}$ of -0.3. Encouraged by these results, we continue to investigate the behavior of chemical abundances in NIHAO simulations.
...
\textbf{Key Takeaway:} While simple abundance-metallicity plots do not easily reveal accreted structures for most elements, both C and Ba abundances show the potential to identify different abundance sequences - if the nucleosynthetic processes of the simulations is accurate.
