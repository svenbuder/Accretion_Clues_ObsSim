...
In Sec.~\ref{sec:intro}, we have reviewed several observation-based plots that have been used to identify accreted stars in the Milky Way. In this section, we are now assessing if the simulated Milky Way analogue shows similar signatures when using these diagnostic plots.
...
\begin{figure}
    ...
    \textbf{Panel a):} iron vs. silicon abundances as tracers of contribution from SNIa and SNII. \textbf{Panel b):} abundance combination [Al/Fe] vs. [Mg/Mn] used to trace accreted stars in observations. The selection polygon in panel a) is drawn for $\mathrm{([Fe/H],[Si/Fe])} \in {[(-0.7,0.24),(-0.5,0.22),(-0.5,0.23),(-0.7,0.25)]}$. A convex hull is then used for panel b) to plot the area of the stars selected via a polygon in panel a).}
    \label{fig:low_alpha_halo}
\end{figure}

\subsection{Location matters: Full galaxy versus Solar neighborhood} \label{sec:location}

Naïvely, we initially plotted the diagnostic plots of observers, namely the ratio of SNII to SNIa via [Si/Fe] as function of iron abundance [Fe/H] (Fig.~\ref{fig:low_alpha_halo}a) as well as the previously successful selection plane of [Al/Fe] vs. [Mg/Mn] (Fig.~\ref{fig:low_alpha_halo}b) for the complete Milky Way analogue. Our hope was, that in the absence of observational noise, we would be able to identify clearly separated areas of in-situ and accreted stars. To our big surprise, the [Al/Fe] vs. [Mg/Mn] showed seemingly even more scatter than the observations. Tracing the selection of accreted stars, that is, the stars with lower [Si/Fe] around $\mathrm{[Fe/H]} \sim -0.6$ with a selection box, does not lead to a unique location in the [Al/Fe] vs. [Mg/Mn] in Fig.~\ref{fig:low_alpha_halo}b when showing the extend of these stars with a convex hull outline.

Before giving up on our endeavour, we applied a similar selection to NIHAO as would be expected from GALAH observations, in particular the limiting of the sample to be within $4.2\,\mathrm{kpc}$ of the Solar radius while leaving out significant fractions of the galactic plane. Surprisingly, both the [Fe/H] vs. [Si/Fe] plane in Fig.~\ref{fig:low_alpha_halo}d as well as the [Al/Fe] vs. [Mg/Mn] plane in Fig.~\ref{fig:low_alpha_halo}e cleared up significantly, with the accreted sequence becoming more prominent around $\mathrm{[Al/Fe]}$ of -0.3. Encouraged by these results, we continue to investigate the behavior of chemical abundances in NIHAO simulations.

Intrigued by the difference in clarity of the chemical planes, we also show two age-metallicity relations - or more accurately age-[Fe/H] planes - for the full Milky Way analogue in Fig.~\ref{fig:low_alpha_halo}c and the smaller footprint to mimic observations in Fig.~\ref{fig:low_alpha_halo}f. The clearly arising separations of sequences in both selections motivates a more dedicated analysis of this plane, as will follow in Sec.~\ref{sec:Age-abundance}. Under the section theme of identifying differences in the planes of full and limited sample selection, we notice a slightly larger scatter of abundances in Fig.~\ref{fig:low_alpha_halo}c, that is, a differences of iron abundances outside the Solar vicinity, as well as the disapearence of a metal-poor ($\mathrm{[Fe/H]} \sim -1.5$) sequence below $10\,\mathrm{Gyr}$ when limiting the selection to the GALAH footprint.
...
In Fig.~\ref{fig:NaFe_MgMn_selection_Age_FeH_dissection}, we contrast the simulations with the observations. When using the same axis limits for simulation (Fig.~\ref{fig:NaFe_MgMn_selection_Age_FeH_dissection}a) and observation (Fig.~\ref{fig:NaFe_MgMn_selection_Age_FeH_dissection}c), we notice the strong offset and significantly smaller spread of abundances in the simulation, in agreement with our previous findings for abundances in Sec.~\ref{sec:feh_xfe}. When zooming into the [Al/Fe] vs. [Mg/Mn] regime for the simulations, however, we note a better qualitative agreement, in that we see two significant overdensities for simulated and observed galaxy. For the observations, we identify the lower and upper overdensity at [Mg/Mn] of 0.0 and 0.4. These have previously been confirmed as low- and high-$\upalpha$-sequence (or thin and thick disk), respectively \citep{Hawkins2015, Buder2022}. These sequences show similar Solar or even enhanced [Na/Fe] and [Al/Fe]. In contrast to this, the major overdensity in the simulation is found around (-0.25,-0.5) with tails reaching higher [Mg/Mn]. The minor overdensity is found towards higher [Mg/Mn] as well as lower [Al/Fe] - coinciding with the expected region of accreted stars \citep{Horta2021} which are less numerous in the observational data of GALAH DR3.

\textbf{Key Takeaway:} The abundance-abundance plot of [Al/Fe] vs. [Mg/Mn] show qualitative agreement with the observational data, but with significant abundance offsets and a more pronounced overdensity of accreted stars than the observations.

\section{A new angle: age-abundance-distributions}\label{sec:Age-abundance}

Fortunately, chemistry is not the only conserved property of stars. We therefore turn to stellar ages and assess age-abundance trends in the data. Observations still suffer from incredibly large uncertainties on the order of $30-50\%$ for most stars, so examining simulation plots removes this barrier and allow us to determine chemical enrichment as a function of age. We note here that age is based on the age of the simulated universe being 14.14 Gyr \citep{Buck2021}.

We first turn to the age-[Fe/H] relationship - more commonly and colloquially referred to as age-metallicity relation - as it has proven to be a powerful tool not only for resolving processes with observed data of our Milky Way \citep[e.g.][]{Twarog1980, Edvardsson1993, Nordstroem2004, Casagrande2011, Feuillet2019, Xiang2022}, but also galaxies - for example to identify accreted stars \citep[e.g.][]{Martig2021}.

Fig.~\ref{fig:AlFe_MgMn_selection_Age_FeH_dissection_sim}
...
\input{tables/separation_significance_two_sequences} % \label{tab:r_values_simulation}
