...
\SB{Note interesting points regarding the linear $N_{X/H}$ plots:
\begin{itemize}
...
\SB{Describe how Fig.~\ref{fig:histograms_xh_in_age_bins_Mn}} is calculated.
...
Assume no observational noise, we calculate $r_{12}$ based only on the means and standard deviations for the age bins and elements {[X/H]} listed in Tab.~\ref{tab:tabular_separation_r12} for the sequences defined in Eqs.~\ref{eq:sequence1}-Eqs.~\ref{eq:sequence3}. For completeness, we also list the properties of Sequence 3, when more than 100 particles are present.
...
\begin{itemize}
    \item intrinsic scatter in {[X/H]} is larger ($\sim 0.1-0.4\,\mathrm{dex}$) for older stars as a result of larger change in this logarithmic scale when compared to youngest stars ($\sim 0.02-0.05\,\mathrm{dex}$). Also shown in top rows of Fig.~\ref{fig:r12_for_sigmaobs}.
    \item Table~\ref{tab:tabular_separation_r12} shows us that the most significant separations $r_{12} > 10$ in the age-[X/H] space are expected among the youngest accreted stars.
    \item Separation significance decreases in order \input{tex_text/r12_sorted.tex}.
    \item In the absence of age uncertainties, we would expect to tell apart Sequence 1 and 2 by $2\sigma$ for $\sigma_\mathrm{obs} < 0.15\,\mathrm{dex}$ (Fig.~\ref{fig:r12_for_sigmaobs}, bottom row).
\end{itemize}

\textbf{Key Takeaway:} In the simulation, accreted and in-situ components separate most significantly among youngest stars across a common age range.
...
If you are using either of these data to follow up on this research, remember to give appropriate credit to the researchers who created and curated either data set, that is, at least to \citet{Buder2021} and \citet{Buck2021}.
