...
We then must consider the second facet of uncertainty: the age uncertainties of the data. We expect the sequences will be significantly more difficult to distinguish in older stars with corresponding larger uncertainties. This is plotted in Fig.~\ref{fig:age_sigma_vs_r}, using a subset of data within $8.5-10\,\mathrm{Gyr}$. This age subset is used as it is the range within which there is the clearest separation between the two populations in the simulated plots (Fig.~\ref{fig:r12_for_sigmaobs}).
...
Because we are working within the age range of $8.5-10\,\mathrm{Gyr}$, this means that a $2\,\mathrm{Gyr}$ uncertainty is equivalent to just over 20\%. We can see from the plot that we require an uncertainty of less than 15\% to achieve the highest separation significance $r$-value.

Our tests on the influence of abundance and age uncertainties, however, neglects an important both are present at the same time in observations. Guided by our previous results on abundance and age uncertainties, and with the actual uncertainties of GALAH DR3 in mind, we therefore have added both noises to the properties of the simulated galaxy and replicated the age-[Fe/H] in Fig.~\ref{fig:nihao_with_scatter}. The figure shows the extremes of no noise on the left (Fig.~\ref{fig:nihao_with_scatter}a) and the actual observed Milky Way age-[Fe/H] relation in the Solar neighbourhood (Fig.~\ref{fig:nihao_with_scatter}f), with different realisations of noise in panels b (10\% age uncertainty, $0.025\,\mathrm{dex}$ noise on [Fe/H]), panels c (15\% for age, $0.05\,\mathrm{dex}$ for [Fe/H]), panels d (30\% for age, $0.075\,\mathrm{dex}$ for [Fe/H], and panels e (50\% for age, $0.15\,\mathrm{dex}$ for [Fe/H]).
...
Fig.~\ref{fig:r_v_age},
Fig.~\ref{fig:nihao_with_scatter}, and 
