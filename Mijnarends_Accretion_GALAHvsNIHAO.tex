...
\begin{figure*}
    ...
    \caption{
    \textbf{Separation significance $r_{12}$ between Sequences 1 and 2 across different elements (panels) as a function of age bins (top panels) and added observational noise (bottom panels).} Lines are calculated for added observational noise in the top panels and different age bins in the bottom panels, respecitvely, as indicated in the legend.}
...
\textbf{Key Takeaway:} The abundance vs age plots are clearest when the abundance is relative to H rather than Fe. Establishing the relationships relative to the number density $N_X$ should also be explored in future work as they bear to potential to allow the tracing of trace linear and non-linear contributions through chemical yields (Fig.~\ref{fig:Fe_H_lin_time}).

\subsection{Quantifying differences of age-{[X/H]} sequences} \label{sec:ageabundance}
