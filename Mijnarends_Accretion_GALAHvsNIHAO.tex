...
\begin{figure}
    ...
    \caption{
    \textbf{Separation significance $r_{12}$ against age uncertainty (in Gyr) for stars with ages between $8.5-10\,\mathrm{Gyr}$ for all elements.
    } An age uncertainty of $2\,\mathrm{Gyr}$ uncertainty for this sample would correspond to just over 20\%.}
...
While being a possibly insightful gedankenexperiment, our tests on the influence of abundance and age uncertainties, however, neglect an important fact. That is, that both uncertainties are present at the same time in observations in more complicated ways and a selection like ours with prior knowledge of the sequence positions is not possible for the Milky Way. Guided by our previous results on abundance and age uncertainties, and with the actual uncertainties of GALAH DR3 in mind, we have added both noises to the properties of the simulated galaxy and replicated the age-[Fe/H] in Fig.~\ref{fig:nihao_with_scatter}, an idea that was also neatly implemented by \citet[][see their Fig.~12]{Renaud2021} to create more realistic noisy simulation data. Our figure shows the extremes of no noise on the left (Fig.~\ref{fig:nihao_with_scatter}a) and the actual observed Milky Way age-[Fe/H] relation in the Solar neighbourhood (Fig.~\ref{fig:nihao_with_scatter}f), with different realisations of noise in panels b (10\% age uncertainty, $0.025\,\mathrm{dex}$ noise on [Fe/H]), panels c (15\% for age, $0.05\,\mathrm{dex}$ for [Fe/H]), panels d (30\% for age, $0.075\,\mathrm{dex}$ for [Fe/H], and panels e (50\% for age, $0.15\,\mathrm{dex}$ for [Fe/H]).

\SB{CONTINUE HERE}

\SB{Discuss Fig.~\ref{fig:nihao_with_scatter}b-e!}
...
\textbf{Key Takeaway:} In the absence of age uncertainties, we would expect to tell apart Sequence 1 and 2 by more than $2\sigma$ for $\sigma_\mathrm{obs} < 0.15\,\mathrm{dex}$ for in-situ and accreted stars younger than $12\,\mathrm{Gyr}$ (Fig.~\ref{fig:r12_for_sigmaobs}, bottom row). We find that GALAH data has sufficient element measurement precision, but significantly too high age uncertainties. When assessing only age uncertainties, we find a significant decrease in separation significance $r_{12}$, when we cannot select ages better than 20\% (or $2\,\mathrm{Gyr}$ for stars with ages $8.5-10\,\mathrm{Gyr}$).
